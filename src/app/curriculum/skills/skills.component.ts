...
export class SkillsComponent implements OnInit {
  ...
  skills: Skill[]
  constructor(private skillsService: SkillsService, private curriculumService:CurriculumService) {}
  ...
  async ngOnInit() {
    ...
    this.curriculumService.currentCurriculum$.subscribe(async curriculum => {
      ...
      if(curriculum.idCurriculum != 0){
        this.skills = await this.skillsService.getSkills(curriculum.idCurriculum)
        this.buildCharts();
      }
    })
  ...
  buildCharts(){
    ...
    for(let skill in this.skills){
      ...
      const knowledgeLevels = skillData.conocimientos.map(conocimiento => conocimiento.nivel);
      const knowledgeNames =  skillData.conocimientos.map(conocimiento => conocimiento.nombre);
